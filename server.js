...
app.post("/produce", async (req, res) => {
  try {
    ...
    if (!cropName || quantity == null || !harvestDate) {
      return res.status(400).json({ error: "Missing fields: cropName, quantity, harvestDate are required." });
    }
    const hash = await walletClient.writeContract({
      address: contractAddress,
      abi,
      functionName: "addProduce",
      args: [String(cropName), BigInt(quantity), String(harvestDate)],
    });
...
app.post("/transfer", async (req, res) => {
  try {
    ...
    if (batchId == null || !to || price == null) {
      return res.status(400).json({ error: "Missing fields: batchId, to, price are required." });
    }
    const hash = await walletClient.writeContract({
      address: contractAddress,
      abi,
      functionName: "transferOwnership",
      args: [BigInt(batchId), to, BigInt(price)],
    });
    const receipt = await publicClient.waitForTransactionReceipt({ hash });
    return res.json({ txHash: hash, status: receipt.status });
  } catch (e) {
